          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-slate-500">No hay solicitudes creadas aún.</td>
                </tr>
              ) : (
                solicitudes.map((req) => (
                  <tr key={req.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-semibold text-slate-800">SC-{req.folio?.toString().padStart(4, '0') || 'N/A'}</div>
                    ...
                    <td className="px-6 py-4">
                      <div className="text-slate-700">{req.solicitante?.nombre}</div>
                    </td>
                    <td className="px-6 py-4">
